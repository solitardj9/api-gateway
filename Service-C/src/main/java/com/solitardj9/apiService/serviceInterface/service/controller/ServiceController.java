...
@RestController
@RequestMapping(value="/service/service-a/b2b")
@CrossOrigin(origins = "*")
public class ServiceController {
	...
	@PostMapping(value="/{b2b}/api/{api}")
	public ResponseEntity onPutService(@PathVariable("b2b") String b2b,
									   @PathVariable("api") String api,
									   @RequestHeader HttpHeaders headers,
									   @RequestBody(required=false) String requestBody) {
		//
		logger.info("[ServiceController].onPutService(Service = service-a, b2b = " + b2b + ", api = " + api + ") is called.");
		
		Response response = serviceHandler.handleService("service-a", b2b, api, headers, requestBody);
